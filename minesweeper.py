...
class MinesweeperAI():
    ...
    def add_knowledge(self, cell, count):
        ...
        for x in nearby_cells:
            if (x in self.safes) or (x in self.mines):
                nearby_cells.remove(x)
    
        self.knowledge.append(Sentence(nearby_cells, my_count))
        
        for sent in self.knowledge:
            if sent.known_mines():
                for cel in list(sent.known_mines()):
                    sent.mark_mine(cel)
            if sent.known_safes():
                for thing in list(sent.known_safes()):
                    sent.mark_safe(thing)
        
        for a in self.knowledge:
            for b in self.knowledge:
                if a.cells.issubset(b.cells):
                    temp_cells = b.cells - a.cells
                    temp_count = b.count - a.count
